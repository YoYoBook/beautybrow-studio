  ...
  const sections = [
    {
      ...
      items: [
        ...
        '需支付定金以保留時段，定金將於施作當日折抵消費',
        '<strong>預約當日</strong>臨時取消、未到、遲到超過 15 分鐘（超過保留時間），<strong>定金將不予退還，亦不提供改期或扣抵使用</strong>',
        '為確保服務品質與美感達成共識，若現場設計階段雙方未達成一致，將不進行後續施作。<strong>原定金將轉為當日之「專業眉型設計」與「技術諮詢服務費」，恕不退還</strong>，敬請見諒。'
      ]
    },
    {
      title: '到店須知',
...
function renderFAQRemoval(config) {
  ...
  const faqs = [
    ...
    },
    {
      q: '能保證 100% 乾淨嗎？',
      a: `誠實說：無法保證<br>
          因為受限於舊色料成分＆入針深度，頑固色素可能還是會留下一點影子。<br><br>
          不過請放心！我的目標是做到「有效淡化」<br>
          大部分客人在多次操作後都能淡化到很不明顯<br>
          視覺上就會乾淨很多，不會影響之後畫眉毛，或是重新設計新的眉型蓋過去，這部分是可以期待的唷`
    }
  ];
